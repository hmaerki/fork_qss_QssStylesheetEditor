...
class Qsst:
    ...
    def __init__(self, qssFile=None):
        ...
        self.varUndefined = (
            []
        )  # var used but not defined，these vars will be assign to ''
        self.codeBlocks = []

        # 嵌入代码定义
        self.reCodeBlock = re.compile(
            r"~{3}.*[\r\n]+(.*)[\r\n]+~{3}|`{3}.*[\r\n]+(.*)[\r\n]+`{3}", re.DOTALL
        )
            ...
            for v in self.varDict:
                if v in varDict.keys():
                    ...
                    qssStr = re.sub(
                        r"[$](\w+)[%](\w+)([\s;]*)",
                        lambda m: "{}{}".format(
                            self.addColorOpacity(varDict[m.group(1)], m.group(2)),
                            m.group(3),
                        ),
                        qssStr,
                    )
                    ...
                    qssStr = re.sub(
                        r"[$](\w+)([\s;]*)",
                        lambda m: "{}{}".format(varDict[m.group(1)], m.group(2)),
                        qssStr,
                    )
                else:
    ...
    def writeVars(self):
        ...
        if self.varDict:  # 如果文件中变量不为空，更新变量值
            self.srctext = re.sub(
                r"[$](\w+)\s*=[ \t]*([#(),.\w]*)[\t ]*[;]?",
                lambda m: "${} = {};".format(
                    m.group(1), varDictNew.get(m.group(1), "")
                ),
                self.srctext,
            )
